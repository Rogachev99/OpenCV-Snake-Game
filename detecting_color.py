...
DETECT_COLOR_LOWER = 136,  87,  111
DETECT_COLOR_UPPER = 180, 255, 255

# function for detecting red color
def detect_color(hsv):
    hsv = cv2.GaussianBlur(hsv, (15, 15), 2)
    mask1 = cv2.inRange(hsv, np.array(DETECT_COLOR_LOWER), np.array(DETECT_COLOR_UPPER))
...
def start_game():
    ...
    while 1:
        ...
        # drawing border
        cv2.rectangle(blank_img, (0, 0), (640, 480), border_color, border_size)
        ...
        cv2.putText(frame, str("Score - "+str(score)), (250, 450),
    ...
    cv2.putText(frame, str("Game Over!"), (100, 230),
                font, 3, (255, 0, 0), 3, cv2.LINE_AA)
    cv2.putText(frame, str("Press any key to Exit."), (180, 260),
                font, 1, (255, 200, 0), 2, cv2.LINE_AA)
    cv2.imshow("frame", frame)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
# restart
